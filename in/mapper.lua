...
-- @param gamepad Optional gamepad index to bind input to
-- @return Any previous action bound to the input
function M.bind(input, action, player, gamepad)
	assert(input)
	assert(action)
...
-- @param gamepad Optional gamepad index the input originated from
-- @return The bound action or nil if no binding exists
function M.on_input(input, player, gamepad)
	if not input then
		return nil
	end
	local bindings = get_player_bindings(player)
	local binding = bindings[input]
	if not binding then
		return nil
	end
